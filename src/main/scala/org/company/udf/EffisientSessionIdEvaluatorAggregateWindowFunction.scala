...
case class EffisientSessionIdEvaluatorAggregateWindowFunction(evaluateNewSessionIdExpr: Expression) extends AggregateWindowFunction {
...
object EffisientSessionIdEvaluatorAggregateWindowFunction {

  def calculateSession(evaluateSessionId: Column): Column = withExpr {
    EffisientSessionIdEvaluatorAggregateWindowFunction(evaluateSessionId.expr)
  }
